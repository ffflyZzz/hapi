...
const RELEASE_URL = 'https://github.com/ffflyZzz/hapi/releases';
...
function getBinaryPath(platformName = platform, archName = arch) {
    const pkgName = `@aiflyzx/hapi-${platformName}-${archName}`;
...
function reportMissingPlatformPackage(platformName = platform, archName = arch, log = console.error) {
    const platformPackage = `@aiflyzx/hapi-${platformName}-${archName}`;
    ...
    log(`  npm install -g @aiflyzx/hapi --registry=${OFFICIAL_NPM_REGISTRY}`);
    log('');
